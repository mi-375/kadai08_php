...
if ($status == false) {
  ...
  exit("ErrorQuery:".$error[2]);
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $view .= '<a href="detail.php?id=' . h($result['id']) . '" class="link-button">編集</a>';
        $view .= '<a href="delete.php?id=' . h($result['id']) . '" class="link-button" onclick="return confirm(\'本当に削除しますか？\');">削除</a>';
        $view .= '</div>';
